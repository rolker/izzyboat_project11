...
dirty sub-windows (up to 960×960 cells × 2 bytes = 1,843,200 B ≈ 1.84 MB for a single INT16
...
3. **Add a link-budget comment block** — document the tile size upper bound
   (worst-case ~1.84 MB/tile for a full 960×960 INT16-depth update) and how
...
but the worst-case message (~1.84 MB) still exceeds what the link comfortably
...
| A change includes its consequences | Single file (`bizzyboat.yaml`); `operator.yaml` pair untouched by design (no routing change); at `period: 1.0` the worst-case offered load is 1,843,200 B/s (one full 960×960 INT16 tile per second) against the 1,500,000 B/s shared VPN cap — still clip-able on a worst-case tile (`queue_size` only buffers, it does not change the rate), which is why the structural bound (cube#112) is paired |
...
| Worst-case message size unchanged (~1.84 MB) | Structural bound belongs at the publisher — paired cube_bathymetry follow-up issue (chunked patches), cross-ref camp#170 | Follow-up issue, filed with this plan |
