...
Copilot code review**, which reads this file when reviewing PRs. The deep
guide (packages, layout, pitfalls) is
...
## Review Context — unh_echoboats_project11

- **Platform configuration for the EchoBoat fleet** (bizzyboat = EchoBoat
  240, currently deployed; izzyboat = 160). Mostly launch files, parameter
  overlays, and deployment logs — but changes take effect on a live boat, so
  topic/param renames ripple directly into field operations.
- **`bizzy` is a ROS namespace, not a host**: boat computers are `gabby`
  (Linux/ROS) and `mercat` (Windows/QINSy).
- **Field-tuned overlay values are deliberate**: parameters here often
  intentionally differ from upstream package defaults — "differs from
  default" is not a finding without an operational argument.
- **Field-import PRs** (reconciled from the gitcloud field remote) are
  drafts by policy: verify topic names and idempotency, expect tests to be
  added at import time, and don't flag raw per-host deployment logs under
  `docs/logs/YYYY/` for polish — they are intentionally raw field records.
