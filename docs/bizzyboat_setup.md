...
**Verification**:
- Ping BenCloud → BizzyBoat (`10.132.146.6`): ~30ms, 0% loss
- Ping salmon → BizzyBoat tunnel IP: working
- `wg show` on BenCloud shows handshake and data transfer
...
**Known issue**: Teltonika does NOT auto-create routes from WireGuard AllowedIPs
(unlike `wg-quick` on Linux). Static routes must be added manually for subnets
beyond the tunnel prefix. See "Missing Static Routes" below.
...
```
iptables -t nat -A PREROUTING -i bcloud -d 192.168.21.0/24 -j NETMAP --to 192.168.20.0/24
iptables -t nat -A POSTROUTING -o bcloud -s 192.168.20.0/24 -j NETMAP --to 192.168.21.0/24
```

Verified: PREROUTING rule matching packets (30 hits observed during testing).
POSTROUTING rule had 0 hits — blocked by missing static routes (see below).
...
### Missing Static Routes — Blocker

Teltonika does **not** auto-create routes from WireGuard AllowedIPs (unlike
`wg-quick` on Linux). The BizzyBoat router's routing table only has
`10.132.146.0/24 dev bcloud` — the tunnel prefix from the interface IP.

Missing routes (need to add via Network → Routing → Static Routes):
- `192.168.22.0/24` dev `bcloud` — BizzyBoat operator NETMAP (return path)
- `192.168.15.0/24` dev `bcloud` — IzzyBoat operator NETMAP

Without these routes, reply traffic from gabby to NETMAP'd operator addresses
(192.168.22.x) goes out the default internet route instead of the WireGuard
tunnel. This is why the POSTROUTING NETMAP rule (which matches on `-o bcloud`)
shows 0 hits.

The same issue may affect the operator router — needs verification that routes
for `192.168.14.0/24` and `192.168.21.0/24` via `bcloud` exist.
...
- [x] NETMAP rules configured (but not yet functional — see missing routes)
- [x] Operator router NETMAP — `xt_NETMAP` loaded after reboot, rules configured
- [ ] **Add static routes on BizzyBoat for 192.168.22.0/24 and 192.168.15.0/24 via bcloud**
- [ ] Verify operator router has static routes for 192.168.14.0/24 and 192.168.21.0/24 via bcloud
- [ ] Test end-to-end NETMAP (salmon → gabby at 192.168.21.5)
