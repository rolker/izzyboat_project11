...
- Endpoint port: `51820`
- AllowedIPs: `10.132.146.0/24`, `192.168.14.0/24`, `192.168.15.0/24`, `192.168.21.0/24`, `192.168.22.0/24`
...
### BizzyBoat Router (Teltonika RUTX11) — configured (2026-03-23)

Configured via Teltonika web UI (SSH tunnel through salmon/WiFi bridge) at
172.16.20.1 under Services → VPN → WireGuard.

**Interface** (instance name: `bcloud`):
- Enable: on
- Private key: _(generated by Teltonika UI; in private repo)_
- IP address: `10.132.146.6/24`
- Listen port: 51821 (changed from 51820 on 2026-03-24 to avoid NAT collision
  with operator router — both share the same public IP)
- WireGuard interface name on Linux: `bcloud`
- Route Allowed IPs: on (auto-creates routes, replacing manual static routes)

**Peer** (name: `bencloud`):
- Public key: `7z8/BYsq/MVKo6VlVktRpqzhle3JMYFiPTFfqHrtz3Q=`
- Endpoint host: `18.213.242.76`
- Endpoint port: `51820`
- AllowedIPs: `10.132.146.0/24`, `192.168.15.0/24`, `192.168.22.0/24`
- Persistent keepalive: 25

**Verification** (2026-03-24):
- End-to-end NETMAP: deadpool → gabby (`ping 192.168.21.5`): ~37ms, 0% loss ✓
- Reverse path: gabby → deadpool (`ping 192.168.22.143`): ~37ms, 0% loss ✓
- Survives power cycle: verified after full boat reboot

**Note**: The Teltonika router generated a new key pair. The original pre-generated
key (`DNdXd+...`) was replaced; BenCloud's `wg0.conf` was updated with the new
public key (`s+TxBz5p...`). Key material in private repo.

**Tunnel debugging (2026-03-24)**: After the BizzyBoat router settings were changed
(listen port, Route Allowed IPs enabled), the tunnel would not establish — BenCloud's
WireGuard silently dropped handshake initiations despite packets arriving (confirmed
via tcpdump). Restarting WireGuard on BenCloud (`wg-quick down wg0 && wg-quick up wg0`)
resolved the issue. Root cause likely stale WireGuard kernel state on BenCloud.
...
##### NETMAP rules — resolved (2026-03-23)

Previously blocked: firmware 00.07.21.2 dropped `xt_NETMAP` from the base image.
The "IPtables NAT extra" package was already installed via Package Manager but
the module was not loaded. A **reboot** resolved the issue — `xt_NETMAP` loaded
successfully after restart.

Custom firewall rules (Network → Firewall → Custom Rules):
...
Verified: rules loaded and matching packets in both PREROUTING and POSTROUTING chains.
...
## BizzyBoat Router — NETMAP Rules (2026-03-23)
...
Verified: both rules matching packets. End-to-end NETMAP confirmed working in
both directions (2026-03-24). Changed from `-A` (append) to `-I` (insert) on
2026-03-24 to ensure NETMAP rules are evaluated before other NAT rules.

### Firewall / Routing — LAN1 ↔ wifi_bridge

No additional configuration needed. The `wifi_bridge` interface (`eth0.3`) is in
the `lan` firewall zone. Salmon can ping gabby (192.168.20.5) directly via the
WiFi bridge path without any extra forwarding rules.

### Static Routes — Resolved (2026-03-24)

Initially, the Teltonika did not auto-create routes from WireGuard AllowedIPs
(unlike `wg-quick` on Linux). Static routes were added manually on 2026-03-24:
- `192.168.22.0/24` dev `bcloud`
- `192.168.15.0/24` dev `bcloud`

Later the same day, the manual static routes were replaced by enabling the
Teltonika "Route Allowed IPs" setting on the WireGuard interface, which
auto-creates routes from the peer's AllowedIPs. This was also applied on the
operator router. Verified working after power cycle.

### Remaining BizzyBoat Router Configuration

- [x] LAN2 as separate interface for WiFi bridge (172.16.20.1/24)
- [x] Firewall / routing between LAN1 and wifi_bridge (already in lan zone)
- [x] WireGuard configuration (instance `bcloud`, tunnel up)
- [x] NETMAP rules configured and verified working (changed `-A` to `-I` on 2026-03-24)
- [x] Operator router NETMAP — `xt_NETMAP` loaded after reboot, rules configured
- [x] Static routes — replaced by WireGuard "Route Allowed IPs" on both routers (2026-03-24)
- [x] End-to-end NETMAP verified both directions (2026-03-24)
- [x] WAN interface re-enabled (2026-03-24; subnet conflict no longer applies)
