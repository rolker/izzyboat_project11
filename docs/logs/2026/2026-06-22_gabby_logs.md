...
> **Wrap-up correction (operator interview 2026-06-22)**: This and the following ~17:00–17:05 entries are RE-ENGAGEMENT timestamps, not real-time observations. The operator left the site ~15:30 and the field AI agents were IDLE during the absence; the agent only resumed (and logged) when the operator re-engaged remotely ~17:00. The 1800 s (30-min) capture that started 15:06:41 actually completed ~15:37 — the 17:00 stamp is when the agent recorded it on re-engagement. Likewise the "boat at the dock" notes below reflect the operator's remote screenshooter check (~17:00), NOT an agent-witnessed return. The students (with coworkers) finished the survey and recovered the boat; the actual dock-return time is unknown and is to be recovered from the GPS track at post-processing.
