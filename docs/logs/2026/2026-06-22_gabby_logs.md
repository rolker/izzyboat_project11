# 2026-06-22 — gabby log (BizzyBoat deployment #309)

Deployment issue: https://github.com/rolker/unh_echoboats_project11/issues/309
<!-- backfilled at wrap-up: issue-less field start (#533) reconciled to #309 -->
Host: gabby
Side: field
...
> **Wrap-up correction (operator interview 2026-06-22)**: Operator is NOT sure the lookahead change was the cause and endorsed the agent's hedge. Fuller sequence: the boat had stopped a few tracklines SHORT of the survey pattern and gone into a HOVER; that hover prompted the operators to DRAW a new trackline; the boat followed the drawn trackline OK, and the erratic behavior began a few SEGMENTS into that drawn trackline (not at the hover transition). Two distinct phenomena to investigate: (a) survey pattern stopping short → hover — operator hypothesis: next survey start point may have been OUTSIDE the local_costmap so the planner couldn't plan to it; (b) erratic following a few segments into the drawn trackline (lookahead_time=3 is one unconfirmed hypothesis among several). See RCA issue filed at wrap-up.
...
> **Wrap-up correction (operator interview 2026-06-22)**: This and the following ~17:00–17:05 entries are RE-ENGAGEMENT timestamps, not real-time observations. The operator left the site ~15:30 and the field AI agents were IDLE during the absence; the agent only resumed (and logged) when the operator re-engaged remotely ~17:00. The 1800 s (30-min) capture that started 15:06:41 actually completed ~15:37 — the 17:00 stamp is when the agent recorded it on re-engagement. Likewise the "boat at the dock" notes below reflect the operator's remote screenshooter check (~17:00), NOT an agent-witnessed return. The students (with coworkers) finished the survey and recovered the boat. GPS (resolved at wrap-up, RTK `gps_pos`) puts the actual return at **16:30:12** — the boat had been docked ~30 min before the ~17:00 remote check. (Cast-off was 09:58:04; a waterline-measurement nudge preceded it ~09:55–09:57.)
